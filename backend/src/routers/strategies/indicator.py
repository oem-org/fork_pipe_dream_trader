...
async def read_all_strategy_indicators(
    ...
    try:
        ...
        strategy = (
            ...
            .options(
                joinedload(Strategies.strategy_indicators).joinedload(
...
def remove_indicator_from_strategy(
    strategy_id: int, indicator_id: int, db: db_dependency, user: user_dependency
):
    strategy_indicator = (
    ...
    if not strategy_indicator:
        raise HTTPException(
            status_code=404, detail="Indicator not connected to strategy"
        )
...
async def update_indicator_in_strategy(
    ...
    try:
        strategy_indicator = (
            db.query(StrategyIndicators)
            .join(Strategies, StrategyIndicators.fk_strategy_id == Strategies.id)
            .filter(
                StrategyIndicators.fk_strategy_id == strategy_id,
                StrategyIndicators.id == indicator_id,
